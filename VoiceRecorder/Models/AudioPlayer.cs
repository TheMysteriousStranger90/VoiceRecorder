...
internal sealed class AudioPlayer : IDisposable
{
    ...
    public void PlayFile(string filePath)
    {
        ...
        try
        {
            ...
            PlaybackStatusChanged?.Invoke(this,
                new PlaybackStatusEventArgs(PlaybackState.Playing, Path.GetFileName(_currentFilePath)));
        }
    ...
    public void PausePlayback()
    {
        if (_soundOut != null && _playbackState == PlaybackState.Playing)
        {
            try
            {
                ...
                PlaybackStatusChanged?.Invoke(this,
                    new PlaybackStatusEventArgs(PlaybackState.Paused,
    ...
    public void ResumePlayback()
    {
        if (_soundOut != null && _playbackState == PlaybackState.Paused)
        {
            try
            {
                ...
                _playbackState = PlaybackState.Playing;
                PlaybackStatusChanged?.Invoke(this,
                    new PlaybackStatusEventArgs(PlaybackState.Playing,
                        Path.GetFileName(_currentFilePath ?? string.Empty)));
            }
            catch (InvalidOperationException ex)
            {
    ...
    private void StopInternal(string? previousFileOverride = null)
    {
        ...
        if (_playbackState != PlaybackState.Stopped)
        {
            _playbackState = PlaybackState.Stopped;
            PlaybackStatusChanged?.Invoke(this,
                new PlaybackStatusEventArgs(PlaybackState.Stopped, fileForEvent ?? string.Empty));
        }
    }
